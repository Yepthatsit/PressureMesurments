...
class LivePlotlyApp:
    ...
    """
    def __init__(self, plot_file_path, status_file_path, predefined_plots, cols, sep, refresh_ms):
        """
    ...
    def run(self, host, port, debug):
        ...
        def update_all(n_intervals, custom_data, selected_cards):
            ...
            """
            df_plot = pd.read_csv(self.plot_file_path, sep=self.sep)
            df_status = pd.read_csv(self.status_file_path, sep=self.sep)
            ...
            for x_col, y_col in self.predefined_plots:
                ...
                fig = px.line(df_plot,x=x_col,y=y_col,title=f"{y_col} vs {x_col}",labels={x_col: x_col, y_col: y_col},markers=True,template=template,color_discrete_sequence=[color])
                fig.update_layout(uirevision='keep_zoom')
                figs.append(fig)

            # Build status cards based on the most recent row
            last_row = df_status.iloc[-1]
            cards = []
            ...
            for pair in custom_data or []:
                color = next(color_cycle)
                fig = px.line(df_plot, x=pair['x'], y=pair['y'], title=f"{pair['y']} vs {pair['x']}", labels={pair['x']: pair['x'], pair['y']: pair['y']},markers=True,template=template,color_discrete_sequence=[color])
                fig.update_layout(uirevision='keep_zoom')
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f1", "--plot-file", required=True, help="Path to CSV file for plotting")
    parser.add_argument("-f2", "--status-file", required=True, help="Path to CSV file for status cards")
